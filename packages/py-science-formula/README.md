...
The restricted parser accepts exact integer and decimal literals, ordinary symbols, arithmetic `+`, `-`, `*`, `/`, and `**`, indexed scalars, bounded nonrecursive `Let(name, value, body)` bindings, ordinary positional calls, one-limit inclusive `Sum(body, (index, lower, upper))`, `Eq(lhs, rhs)`, single relationships, and signed infinity `oo` or `-oo`. A `Let` value sees its enclosing scope but not its own name; its name is visible only in its body, and its value is charged once at lexical placement. It is preserved in normalized syntax and lowered only for exact represented-value mathematics, not runtime or scheduling claims. It rejects unrestricted Python or SymPy, including `Product`, submitted `Max`, attributes, keyword calls, chained relationships, and sums with more than one limit. A generic call can parse while still having no definition, cost, or bounded query semantics; parsing, request-context validation, and evaluator applicability are separate Python-owned checks.
...
Every plan carries canonical objective provenance while its candidate and stable identity exclude objective policy. Adjacent ordering distinguishes proved superiority from deterministic non-superiority; position one makes no superiority claim. Every suggestion has a nonempty tuple of unique target-local transformations. Each transformation owns its target, deterministic child-index occurrence paths, binders and output indices, and normalized original and proposed forms. Atomic cross-equation sharing includes one transformation for every affected equation, including renamed compatible indices. Optional collision-free intermediates, exact identity evidence, conditions and assumptions, selected-objective before, after, and savings, and the `exact_symbolic_only` finite-precision qualification remain suggestion-level. Pi's compact projection presents the selected optimization suggestion's complete transformation set and names additional suggestions in canonical `details`; deterministic presentation is not a superiority claim, and it never invents a primary target. Advice is informational and never changes the retained interpretation, counts, work, scenarios, queries, dependencies, reuse, or extraction diagnostics. Exact-symbolic equivalence is not a runtime, numerical-stability, or identical floating-point evaluation claim.
