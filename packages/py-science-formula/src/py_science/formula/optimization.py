    ...
    *,
    ...
    def visit(
        ...
        if isinstance(node, (BinaryExpression, Call, Sum)) and not is_named_reference:
            occurrences.append(
                _Occurrence(
                    ...
                    path=path,
                    expression=node,
                    free_symbols=frozenset(_free_symbols(node, frozenset(bound))),
                    binders=bound,
                    scope=_EvaluationScope(output_indices, bound),
                )
            )
        if isinstance(node, Sum):
            ...
            visit(node.body, (*path, 2), (*bound, node.index))
            return
    ...
    *,
    ...
    try:
        occurrences = _detect_occurrences(
            target, expression, producers, output_indices=output_indices
        )
