...
The compact ordinary-analysis text shows the selected one- or two-step optimization plan: every ordered family step and affected target, original-to-final selected-objective saving, conditions, and finite-precision qualification. This deterministic presentation is not a superiority claim. Canonical `details` contains every complete replayable trace candidate and identity; search-incomplete and output-truncation qualifications remain separate. Pi correlates and presents protocol-v15 fields but never applies a transformation or recomputes proof, objective, scheduling, or ranking policy.
