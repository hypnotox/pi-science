...
description: Analyze one restricted SymPy expression or named equation system with bounded verified local optimization advice, compare two mapped candidates, or analyze bounded one-axis aggregate-work term dominance, for normalized interpretation, qualified symbolic work, reuse, provenance, bounded mathematical conclusions, and unresolved costs. Use for formula structure and abstract work, not numeric evaluation, benchmarking, physical validation, or implementation generation.
---
...
The compact tool text shows only the first-ranked proved suggestion: every target-local replacement in its atomic transformation set (never a primary target), any shared intermediate, whole-computation abstract-work before/after and savings, conditions, and the `exact_symbolic_only` finite-precision qualification. This deterministic presentation is not a superiority claim. It names additional proved suggestions in canonical `details`; localized incomplete-search guidance and qualifications also remain visible. Read `details` for every target, occurrence path, crossed binder/output scope, exact identity evidence, assumptions, and the complete ordered population. Do not combine separate suggestions; apply transformations together only when Python returns one atomic transformation set.
