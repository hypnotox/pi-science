...
The compact tool text shows only the best proved suggestion: every target-local replacement in its atomic transformation set (never a primary target), any shared intermediate, whole-computation abstract-work before/after and savings, conditions, and the `exact_symbolic_only` finite-precision qualification. It names additional proved suggestions in canonical `details`; localized incomplete-search guidance and qualifications also remain visible. Read `details` for every target, occurrence path, crossed binder/output scope, exact identity evidence, assumptions, and the complete ordered population.
