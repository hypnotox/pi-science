...
The compact ordinary-analysis text shows only the selected optimization suggestion: every target-local replacement in its atomic transformation set (never a primary target), any shared intermediate, selected-objective before, after, and savings, conditions, and the finite-precision qualification. This deterministic presentation is not a superiority claim. Canonical `details` contains every plan and suggestion; localized incomplete-search or failed-advice qualifications also remain visible. Read `details` for every target, occurrence path, crossed binder/output scope, exact identity evidence, assumptions, and the complete ordered population.
