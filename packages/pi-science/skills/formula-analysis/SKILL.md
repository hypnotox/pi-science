...
Submit `operation: optimize` for complete replayable plans. Set `max_plans` to a strict integer from `1` through `16`; omission defaults to three. Each plan contains one complete candidate, its caller-output identities, the same verified diagnostic suggestion used by ordinary advice, and a stable identity. Submit the candidate's expression or equations and mathematical context directly to ordinary analysis or candidate comparison without changing its mathematical content; Pi supplies the restricted-SymPy syntax. Keep plans atomic and do not combine separate candidates. Inspect normalized replay, output identities, conditions, assumptions, work, and `exact_symbolic_only` qualification before using a plan.

A direct `failed` result is a bounded operation failure and contains no unverified plan. `incomplete` means a search or output budget was exhausted, may still contain proved plans, and never means no improvement exists. `complete` with no plans means no candidate qualified within the bounded search. Ordinary expression and equation-system analysis remains default-on advice over the same plans. Set `optimization.max_suggestions` to a strict integer from `0` through `16`; `0` disables advice.

The compact ordinary-analysis text shows only the first-ranked proved suggestion: every target-local replacement in its atomic transformation set (never a primary target), any shared intermediate, whole-computation abstract-work before/after and savings, conditions, and the finite-precision qualification. This deterministic presentation is not a superiority claim. Canonical `details` contains every plan and suggestion; localized incomplete-search or failed-advice qualifications also remain visible. Read `details` for every target, occurrence path, crossed binder/output scope, exact identity evidence, assumptions, and the complete ordered population.

The shipped families are repeated-subexpression extraction, identical-call and reciprocal reuse, checked factoring, redundant-operation removal, iterator-invariant hoisting, compatible sharing across named equation RHSs, and bounded Horner reformulation. Sharing requires one compatible positional free-index interface and acyclic producer placement; Horner stays within fixed variable, degree, term, and node ceilings. Python publishes only independently proved positive whole-computation abstract-work reductions; it omits unknown-cost, unresolved-cardinality, unproved, capture-prone, incompatible-scope, nonpositive, or incomparable candidates. Unconditional suggestions precede assumption-dependent suggestions; incomparable savings retain deterministic non-superiority ordering. Pi validates shape and correlation only and never recomputes algebra, work, applicability, or ranking.
