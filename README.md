...
Ordinary expression and equation-system analysis returns bounded informational optimization advice by default. `optimization.max_suggestions` is a strict integer from `0` through `16`, defaults to `3`, and requests an upper bound; `0` disables advice. A completed empty report found no publishable candidate within the bounded search, while `incomplete` records exhausted inspection, generation, transformation, proof, work, or output capacity and does not mean no improvement exists.
...
Each suggestion carries structural paths and scopes, proof evidence and conditions, selected-objective before, after, and positive savings, and an `exact_symbolic_only` finite-precision qualification. Atomic sharing identifies every affected equation rather than inventing a primary target. It never changes submitted interpretation, ordinary fields, scenarios, queries, dependencies, reuse, or legacy extraction diagnostics. Pi protocol v14 validates and transports this report without recomputing algebra, objective values, ordering, or applicability. Abstract objectives are not runtime, and exact-symbolic reassociation does not establish identical floating-point behavior or numerical stability.
