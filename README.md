...
Ordinary expression and equation-system analysis returns bounded informational optimization advice by default. `optimization.max_suggestions` is a strict integer from `0` through `16`, defaults to `3`, and selects only a final ranked prefix; `0` disables advice. Python always completes the same fixed deterministic depth-two search before that projection. A completed empty report found no publishable candidate within the bounded search, while `incomplete` records search-resource exhaustion and does not mean no improvement exists. Post-search serialized-output truncation is separately reported and never changes search status.

Python keeps eight exact-algebraic families enabled by default: repeated-subexpression extraction, identical-call and reciprocal reuse, checked factoring, redundant-operation removal, iterator-invariant hoisting, compatible cross-equation sharing, and bounded Horner reformulation. Set `optimization.enabled_algorithmic_families` (or the direct request's top-level field) to `["finite_polynomial_sum_v1"]` to add the ninth `exact_algorithmic_v1` lane. It replaces only ADR-0012's unique maximal bounded nested finite-polynomial `Sum` tree inside its surrounding shell. Query candidates remain informational; optimizer proof independently rederives the antidifference and boundaries against each replayed parent and again for direct original-to-final proof. Every candidate must reduce the selected whole-computation objective, including iterator cardinality and output multiplicity. Absent, empty, ineligible, unresolved, or nonpositive algorithmic cases are silent and preserve other analysis.

Omitting `optimization.objective` selects `{"kind":"unit_work_v1"}`. A custom selector uses `{"kind":"weighted_operations_v1","weights":{...}}` with all five strictly positive bounded exact-rational `additions`, `subtractions`, `multiplications`, `divisions`, and `powers` weights; known opaque work keeps coefficient one. The direct optimize operation accepts the same selector at top level. Plans carry canonical objective provenance, selected-objective before/after/savings evidence, and adjacent ordering qualification. Candidates and their stable identities exclude objective policy. Ordinary work, scenarios, comparison, and dominance remain unit-work analysis.

For example, `{"expression":"x*y + x*z"}` can report the independently checked local replacement `x*(y + z)` with default objective `3 → 2`. Compact Pi text presents the selected optimization suggestion's complete atomic target-local replacement set, profile, savings, conditions, used assumptions, and finite-precision qualification. Position one is neutral; later positions distinguish proved adjacent superiority from deterministic non-superiority. Canonical `details` contains every plan.

Every plan carries one or two replayable trace steps. Each step carries its family-local structural paths and scopes, `exact_algebraic_v1` or `exact_algorithmic_v1` tier, proof evidence and conditions, selected-objective before, after, positive savings, complete post-step candidate, and candidate identity; atomic sharing identifies every affected equation rather than inventing a primary target. Plan evidence is independently original-to-final. Python canonically deduplicates complete states for fair breadth-first scheduling only; returned candidate order and identity remain caller-facing. Pi protocol v16 validates and transports parent/child/final correlation without recomputing algebra, objective values, ordering, or applicability. Abstract objectives are not runtime, and exact-symbolic reassociation does not establish identical floating-point behavior or numerical stability.
