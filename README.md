...
For example, `{"expression":"x*y + x*z"}` can report the independently checked local replacement `x*(y + z)` with aggregate abstract work `3 → 2`. Compact Pi text presents the first-ranked proved suggestion's complete atomic target-local replacement set, any shared intermediate, savings, conditions, used assumptions, and finite-precision qualification; deterministic presentation is not a superiority claim, and canonical `details` names additional proved suggestions.

Each suggestion carries structural paths and scopes, proof evidence and conditions, before/after work and positive savings, and an `exact_symbolic_only` finite-precision qualification. Atomic sharing identifies every affected equation rather than inventing a primary target. It never changes submitted interpretation, ordinary fields, scenarios, queries, dependencies, reuse, or legacy extraction diagnostics. Pi protocol v12 validates and transports this report without recomputing algebra, work, ranking, or applicability. Abstract work is not runtime, and exact-symbolic reassociation does not establish identical floating-point behavior or numerical stability.
