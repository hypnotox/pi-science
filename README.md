...
Python currently generates repeated-subexpression extraction, identical-call and reciprocal reuse, checked factoring, redundant-operation removal, iterator-invariant hoisting, compatible cross-equation sharing, and bounded Horner reformulation. Every candidate passes the same exact-symbolic output proof and whole-computation aggregate-work comparison, including iterator cardinality, equation-output multiplicity, and generated-intermediate scope. Shared producers require compatible positional output interfaces and acyclic placement; Horner candidates stay within fixed variable, degree, term, and node ceilings. Unknown costs, unresolved proof or cardinality, incompatible scopes, and nonpositive or incomparable reductions are omitted. Unconditional suggestions precede assumption-dependent suggestions; incomparable savings retain deterministic non-superiority ordering.

For example, `{"expression":"x*y + x*z"}` can report the independently checked local replacement `x*(y + z)` with aggregate abstract work `3 → 2`. Compact Pi text presents the selected optimization suggestion's complete atomic target-local replacement set, any shared intermediate, savings, conditions, used assumptions, and finite-precision qualification; deterministic presentation is not a superiority claim, and canonical `details` names additional proved suggestions.

Each suggestion carries structural paths and scopes, proof evidence and conditions, before/after work and positive savings, and an `exact_symbolic_only` finite-precision qualification. Atomic sharing identifies every affected equation rather than inventing a primary target. It never changes submitted interpretation, ordinary fields, scenarios, queries, dependencies, reuse, or legacy extraction diagnostics. Pi protocol v14 validates and transports this report without recomputing algebra, work, ranking, or applicability. Abstract work is not runtime, and exact-symbolic reassociation does not establish identical floating-point behavior or numerical stability.
