...
Python currently generates repeated-subexpression extraction, identical-call and reciprocal reuse, checked factoring, redundant-operation removal, iterator-invariant hoisting, compatible cross-equation sharing, and bounded Horner reformulation. Every candidate passes the same exact-symbolic output proof and whole-computation objective comparison, including iterator cardinality, equation-output multiplicity, and generated-intermediate scope. Shared producers require compatible positional output interfaces and acyclic placement; Horner candidates stay within fixed variable, degree, term, and node ceilings. Unknown costs, unresolved proof or cardinality, incompatible scopes, and nonpositive selected-objective reductions are omitted.
...
Every plan carries one or two replayable trace steps. Each step carries its family-local structural paths and scopes, proof evidence and conditions, selected-objective before, after, positive savings, complete post-step candidate, and candidate identity; atomic sharing identifies every affected equation rather than inventing a primary target. Plan evidence is independently original-to-final. Python canonically deduplicates complete states for fair breadth-first scheduling only; returned candidate order and identity remain caller-facing. Pi protocol v15 validates and transports parent/child/final correlation without recomputing algebra, objective values, ordering, or applicability. Abstract objectives are not runtime, and exact-symbolic reassociation does not establish identical floating-point behavior or numerical stability.
