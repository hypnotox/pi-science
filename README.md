...
For example, `{"expression":"x*y + x*z"}` can report the independently checked local replacement `x*(y + z)` with aggregate abstract work `3 → 2`. Compact Pi text presents the best proved suggestion's complete atomic target-local replacement set, any shared intermediate, savings, conditions, used assumptions, and finite-precision qualification; it names additional proved suggestions in canonical `details`.
