...
For example, `{"expression":"x*y + x*z"}` can report the independently checked local replacement `x*(y + z)` with aggregate abstract work `3 → 2`; callers must still inspect its proof and finite-precision qualification in `details`.

Each suggestion carries structural paths and scopes, proof evidence and conditions, before/after work and positive savings, and an `exact_symbolic_only` finite-precision qualification. It never changes submitted interpretation, ordinary fields, scenarios, queries, dependencies, reuse, or legacy extraction diagnostics. Pi protocol v12 validates and transports this report without recomputing algebra, work, ranking, or applicability. Abstract work is not runtime, and exact-symbolic reassociation does not establish identical floating-point behavior or numerical stability.
