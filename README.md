...
Ordinary analysis does not run optimization or return plans. Request optimization explicitly with a preserve-all exact-symbolic goal, the fixed search and proof policies, and a projection limit:

```json
{"operation":"optimize","expression":"x*y + x*z","goal":{"kind":"preserve_all_outputs_v1","semantics":"exact_symbolic_v1","operating_domain":"submitted_domain_v1","objective":{"kind":"unit_work_v1"}},"search":{"kind":"bounded_goal_v1"},"proof":{"kind":"verifier_backed_v1"},"projection_limit":3}
```

The goal preserves every submitted output and uses the computation's submitted variable and output domains, constraints, and assumptions. Definitions, functions, and primitive costs remain part of the computation context. A weighted objective uses `{"kind":"weighted_operations_v1","weights":{...}}` with strictly positive bounded exact-rational `additions`, `subtractions`, `multiplications`, `divisions`, and `powers`; known opaque work keeps coefficient one. The request exposes no family, depth, budget, selected-output, goal-local-domain, or hard-resource-bound control.

Python searches all shipped exact-algebraic lanes plus the exact-algorithmic `finite_polynomial_sum_v1` lane with fixed fair monotonic depth two. Every plan carries a replayable one- or two-step trace and only a `strict_improvement` claim under the selected aggregate abstract-work objective. It independently proves every transition and the original-to-final positive whole-computation reduction. Query candidates are not optimizer proof.

Read result fields independently. `classification` describes the observed population as `plans_returned`, `no_applicable_candidate`, or `no_verified_improvement`; it is not an optimality claim. `search_scope.completion` reports bounded search exhaustion. `selection.kind` is only `deterministic_ranked_prefix`, and `projection_status` separately reports output truncation. A blocker records localized missing information observed during existing work, not a candidate, recommendation, or proof. Pi protocol v17 validates, correlates, and presents these Python-owned fields without recomputing mathematics. Neither a completed search nor the first plan proves best-candidate, finite-space, global, runtime, numerical, or finite-precision optimality.
