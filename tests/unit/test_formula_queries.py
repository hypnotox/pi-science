...
def test_lexical_binding_equivalence_uses_represented_value() -> None:
    outcome = analyze(
        AnalysisRequest(
            ...
            queries=({"name": "q", "kind": "equivalence", "comparison": "(x + 1)**2"},),
        )
    )
...
def test_equivalence_comparison_rejects_nonfresh_lexical_binding_name() -> None:
    outcome = analyze(
        AnalysisRequest(
            ...
            expression="x",
            variables={"x": VariableDeclaration(domain=MathematicalDomain.REAL)},
            queries=({"name": "q", "kind": "equivalence", "comparison": "Let(x, 1, x)"},),
        )
    )
    ...
    (
        (
            AnalysisRequest(
                syntax=FormulaSyntax.SYMPY,
                expression="x + 1",
                queries=({"name": "q", "kind": "equivalence", "comparison": "1 + x"},),
            ),
            "x + 1",
        ),
        (
...
def test_retained_analysis_state_is_deeply_read_only(monkeypatch):
    ...
    retained = []
    original_attach = orchestration._attach_queries

    def capture(request, analyzed):
        retained.append(analyzed)
        return original_attach(request, analyzed)

    monkeypatch.setattr(orchestration, "_attach_queries", capture)
    outcome = analyze(
        AnalysisRequest(
            syntax=FormulaSyntax.SYMPY,
            equations=(
...
def test_equivalence_conclusions_and_empty_default():
    ...
    for comparison, conclusion in (
        ("x", "proved"),
        ("x + 1", "disproved"),
        ("Sum(x, (i, 0, 1))", "unresolved"),
    ):
        outcome = analyze(
            request(queries=({"name": "q", "kind": "equivalence", "comparison": comparison},))
        )
        assert outcome.status == "success"
...
def test_assumption_qualification_named_rhs_and_later_consumers():
    qualified = analyze(
        request(
            variables={"x": VariableDeclaration(domain=MathematicalDomain.REAL)},
            assumptions=(Assumption(name="a", relationship="x == 2"),),
            queries=({"name": "q", "kind": "equivalence", "comparison": "2"},),
        )
    )
    ...
    assert system.queries[1].answers[0].blockers == (
        "closed-form expression has no sibling sums; use one to eight sibling (a*k+b)*r**k sums",
    )
...
def test_closed_form_series_rule_matrix_and_afmm_identity():
    afmm = analyze(
        AnalysisRequest(
            ...
            assumptions=(
                Assumption(name="q_nonnegative", relationship="q >= 0"),
                Assumption(name="q_converges", relationship="q < 1"),
            ),
    ...
    assert (
        finite.queries[0].answers[0].derived_candidates[0].interpretation.normalized_sympy == "34"
    )

    for expression, conclusion, blocker in (
        ("Sum(k * 2**k, (k, 0, oo))", "inapplicable", None),
        ("Sum(k * q**k, (k, 0, oo))", "unresolved", "series convergence is not proved"),
        (
            "Sum(Sum(k * q**k, (k, 0, 1)), (j, 0, 1))",
            "unresolved",
            "nested polynomial summand contains forbidden or undeclared names",
        ),
        (
            "Sum(k**2 * q**k, (k, 0, 1))",
            "unresolved",
        ...
        outcome = analyze(
            AnalysisRequest(
                syntax=FormulaSyntax.SYMPY,
                expression=expression,
                queries=({"name": "series", "kind": "closed_form"},),
            )
        )
        assert outcome.status == "success"
        ...
        if blocker is not None:
            assert terminal.blockers == (blocker,)
    empty = analyze(
        AnalysisRequest(
            syntax=FormulaSyntax.SYMPY,
            expression="Sum(k * q**k, (k, 2, 1))",
            queries=({"name": "empty", "kind": "closed_form"},),
        )
    )
...
def test_nested_finite_polynomial_closed_form_is_direct_only():
    ...
    direct_property = analyze(
        AnalysisRequest(
            syntax=FormulaSyntax.SYMPY,
            expression="Sum(Sum(1, (l, -k, k)), (k, 0, p))",
            variables={"p": VariableDeclaration(domain=MathematicalDomain.NONNEGATIVE_INTEGER)},
            queries=({"name": "properties", "kind": "properties", "checks": ({"kind": "sign"},)},),
        )
    )
...
def test_nested_polynomial_shell_topology_and_resource_boundaries():
    variables = {"p": VariableDeclaration(domain=MathematicalDomain.NONNEGATIVE_INTEGER)}
    shell = _nested_answer("3 + 2*Sum(Sum(1, (l, -k, k)), (k, 0, p))", variables=variables)
    ...
    per_binder = _nested_answer("Sum(Sum(k**8*l**8, (l, 0, 1)), (k, 0, 1))")
    ...
    introduced_count = _nested_answer(
        ...
        definitions=(
            DirectedDefinition(
                variable="x",
                expression=" + ".join(f"Sum(1, (i{index}, 0, 1))" for index in range(7)),
            ),
        ),
    )
    ...
    for expression, blocker in (
        (
            "Sum(Sum(Sum(Sum(Sum(1, (a, 0, 1)), (b, 0, 1)), (c, 0, 1)), (d, 0, 1)), (e, 0, 1))",
            "one tree of at most depth four and eight sums",
        ),
        (
            "Sum(Sum(Sum(1, (a, 0, 1)) + Sum(1, (b, 0, 1)), (c, 0, 1)) + Sum(Sum(1, (d, 0, 1)) + Sum(1, (e, 0, 1)) + Sum(1, (f, 0, 1)) + Sum(1, (g, 0, 1)), (h, 0, 1)), (i, 0, 1))",
            "one tree of at most depth four and eight sums",
        ),
        (
            "Sum(Sum(1, (l, 0, k)), (k, 0, 1)) + Sum(Sum(1, (j, 0, i)), (i, 0, 1))",
            "one tree of at most depth four and eight sums",
        ),
...
def test_nested_polynomial_affine_integral_order_and_degree_contract():
    ...
    accepted = _nested_answer("Sum(Sum(p + l**8, (l, 0, 1)), (k, 0, 1))", variables=integer)
    assert accepted.conclusion == "proved"
    dependent = _nested_answer("Sum(Sum(1, (l, k, p)), (k, 0, p))", variables=integer)
    ...
    for expression, blocker, extra in (
        ...
        ("Sum(Sum(1, (l, k/2, k/2)), (k, 0, p))", "affine integers", {"variables": integer}),
        (
            "Sum(Sum(1/(k + 1), (l, 0, 1)), (k, 0, p))",
            "exact rational polynomial",
            {"variables": integer},
        ),
        ("Sum(Sum(k**9, (l, 0, 1)), (k, 0, p))", "degree at most eight", {"variables": integer}),
        (
            "Sum(Sum(y, (l, 0, 1)), (k, 0, p))",
            "capture a bound",
            {
                "variables": {
                    **integer,
                ...
                "definitions": (DirectedDefinition(variable="y", expression="k**9"),),
            },
        ),
...
def test_nested_polynomial_backend_generation_is_never_proof(monkeypatch):
    ...
    assert escaped.blockers == (
        "nested polynomial candidate escapes its restricted names or bounds",
    )
...
def test_nested_polynomial_candidate_and_rendering_overflow_fail_closed(monkeypatch):
    ...
    oversized_target = _nested_answer(f"Sum(Sum({target_terms[0]}, (l, 0, 1)), (k, 0, 1))")
...
def test_nested_polynomial_backend_independently_checks_step_boundary_and_bounds(monkeypatch):
    ...
    assert (
        ...
        and not isinstance(lower, tuple)
        and not isinstance(upper, tuple)
    )
    ...
    monkeypatch.setattr(
        formula_sympy.sympy, "summation", lambda *args: formula_sympy.sympy.Integer(0)
    )
...
def test_nested_polynomial_binders_shadow_declared_definitions():
    ...
    assert captured.blockers == ("nested polynomial reasoning would capture a bound name",)
    ...
    assert transitive.blockers == ("nested polynomial reasoning would capture a bound name",)
...
def test_nested_reasoning_expansion_has_a_shared_budget(monkeypatch):
    ...
    expression = parse_expression(f"Sum(Sum({target_terms[0]}, (l, 0, 1)), (k, 0, 1))")
    ...
    assert falsified.blockers == ("nested polynomial family exceeds its bounded preconditions",)
...
def test_nested_polynomial_canonicalizes_packed_and_literal_m2l_counts():
    ...
    for expression, comparison, canonical, fixed_value, check_sign in cases:
        ...
        assert (
            general.queries[0].answers[0].derived_candidates[0].interpretation.normalized_sympy
            == canonical
        )
        assert all(
            item.answers[0].conclusion in {"proved", "proved_under_assumptions"}
            for item in general.queries
        )
        ...
        assert (
            general.queries[0]
            .answers[0]
            .evidence.statement.startswith(  # pyright: ignore[reportOptionalMemberAccess, reportAttributeAccessIssue]
        ...
        assert {item.name for item in fixed.queries[1].answers[0].assumptions_used} == {
            "fixed_order"
        }
...
def test_nested_polynomial_canonicalization_fails_closed(monkeypatch, attribute, replacement):
...
def test_real_nested_polynomial_canonical_verifier_rejects_wrong_candidate():
    ...
    assert not formula_sympy.bounded_polynomial_canonical_verify(
        expression,
        formula_sympy.sympy.Integer(0),  # pyright: ignore[reportArgumentType]
    )
...
def test_closed_form_uses_affine_facts_domain_obligations_and_bounded_collection():
    ...
    collected = analyze(
        AnalysisRequest(
            syntax=FormulaSyntax.SYMPY,
            expression="Sum(k*q**k + q**k, (k, 0, 2))",
    ...
    unresolved = analyze(
        AnalysisRequest(
            syntax=FormulaSyntax.SYMPY,
            expression="Sum(k*q**k, (k, p, 3))",
            variables={"p": VariableDeclaration(domain=MathematicalDomain.INTEGER)},
            assumptions=(
                Assumption(name="p_before", relationship="p <= 3"),
                Assumption(name="q_not_one", relationship="q < 1"),
            ),
            queries=({"name": "negative", "kind": "closed_form"},),
        )
    )
    assert unresolved.status == "success"
    assert unresolved.queries[0].answers[0].blockers == (
    ...
    discharged = analyze(
        AnalysisRequest(
            ...
            assumptions=(
                Assumption(name="p_before", relationship="p <= 3"),
                Assumption(name="q_nonzero", relationship="q > 0"),
                Assumption(name="q_not_one", relationship="q < 1"),
            ),
            queries=({"name": "negative", "kind": "closed_form"},),
        )
...
def test_closed_form_never_labels_an_unverified_candidate_proved(monkeypatch):
    ...
    monkeypatch.setattr(
        "py_science.formula.series.bounded_series_verify", lambda *args, **kwargs: False
    )
    outcome = analyze(
        AnalysisRequest(
            syntax=FormulaSyntax.SYMPY,
...
def test_closed_form_residual_verification_domain_preflight_normalization_and_provenance(
    monkeypatch,
):
        ...
        extra = (
            {
                ...
                "assumptions": (
                    Assumption(name="converges", relationship="q > 0"),
                    Assumption(name="below_one", relationship="q < 1"),
                ),
            }
            if "oo" in expression
            else {}
    ...
    assert denominator.queries[0].answers[0].blockers == (
        "original denominator is not proved nonzero",
    )
    ...
    provenance = analyze(
        AnalysisRequest(
            syntax=FormulaSyntax.SYMPY,
            expression="Sum(k*r**k, (k, 0, 3))",
            assumptions=(Assumption(name="ratio_value", relationship="r == 2"),),
            queries=({"name": "closed", "kind": "closed_form"},),
        )
    )
    assert provenance.status == "success"
    assert {use.name for use in provenance.queries[0].answers[0].assumptions_used} == {
        "ratio_value"
    }
            ...
            queries=(
                {
                    "name": "q",
                    "kind": "equivalence",
                    "target": {"kind": "equation", "name": "x"},
                    "comparison": "x",
                },
            )
        )
    with pytest.raises(ValidationError):
        request(queries=({"name": "q", "kind": "limit", "variable": "x", "point": "0"},))
    with pytest.raises(ValidationError):
        request(
            queries=(
                {"name": "q", "kind": "limit", "variable": "x", "point": "oo", "direction": "left"},
            )
        )
    with pytest.raises(ValidationError):
        request(queries=({"name": "q", "kind": "properties", "checks": []},))
    with pytest.raises(ValidationError):
        request(
            queries=(
                {"name": "q", "kind": "properties", "checks": [{"kind": "sign", "variable": "x"}]},
            )
        )
    with pytest.raises(ValidationError):
        request(
            queries=(
                {"name": "q", "kind": "properties", "checks": [{"kind": "sign"}, {"kind": "sign"}]},
            )
        )
    for query in (
...
def test_later_consumers_preserve_exact_answer_shape_and_check_order():
            ...
            queries=(
                {
                    "name": "p",
                    "kind": "properties",
                    "checks": ({"kind": "sign"}, {"kind": "valid_domain", "variable": "x"}),
                },
    ...
    assert all(
        answer.derived_candidates == () for result in outcome.queries for answer in result.answers
    )
    ...
    for expression, blocker in (
        ...
        outcome = analyze(
            AnalysisRequest(
                ...
                expression=expression,
                queries=(
                    {"name": "a", "kind": "asymptotic", "variable": "x", "point": "oo", "order": 1},
                ),
            )
        )
        assert outcome.status == "success"
...
def test_result_models_reject_wrong_evidence_and_answer_cardinality():
    ...
    with pytest.raises(ValidationError):
        QueryAnswer(
            ...
            derived_candidates=(
                {"interpretation": interpretation, "operation_counts": OperationCounts()},
            ),
        )
...
def test_provenance_population_refusal_does_not_escape_analyze():
    ...
    outcome = analyze(
        AnalysisRequest(
            ...
            assumptions=tuple(
                Assumption(name=f"a{index}", relationship=f"y{index} == x") for index in range(128)
            ),
...
def test_equivalence_reports_operand_expansion_and_normalization_refusals(monkeypatch):
    operand = analyze(
        request(queries=({"name": "q", "kind": "equivalence", "comparison": "sin(x)"},))
    )
    ...
    normalization = analyze(
        request(queries=({"name": "q", "kind": "equivalence", "comparison": "x"},))
    )
...
def test_query_sources_participate_in_whole_request_byte_accounting():
    functions = tuple(
        FunctionDefinition(name=f"f{index}", parameters=(), body="x" * 65_000) for index in range(4)
    )
    outcome = analyze(
        AnalysisRequest(
            syntax=FormulaSyntax.SYMPY,
...
def test_derived_results_inherit_the_named_source_qualification_and_preserve_terminal_details():
    ...
    source = ClosedFormResult(
        ...
        answers=(
            QueryAnswer(
                ...
                relevant_unsupported_assumptions=("source unsupported",),
    ...
    dependent = EquivalenceResult(
        name="dependent",
        target=DerivedTarget(query="closed"),
...
def test_derived_qualification_overflow_is_unresolved_without_losing_existing_details():
    ...
    dependent = EquivalenceResult(
        ...
        answers=(
            QueryAnswer(
                conclusion="inapplicable",
                relevant_unsupported_assumptions=("dependent",),
                blockers=("derived target source closed concluded unresolved",),
            ),
        ),
    )
        ...
        (
            {
                "name": "later",
                "kind": "equivalence",
                "target": {"kind": "derived", "query": "missing"},
                "comparison": "x",
            },
        ),
        (
        ...
        (
            {
                "name": "self",
                "kind": "equivalence",
                "target": {"kind": "derived", "query": "self"},
                "comparison": "x",
            },
        ),
        (
            {"name": "not_closed", "kind": "equivalence", "comparison": "x"},
            {
                "name": "dependent",
                "kind": "equivalence",
                "target": {"kind": "derived", "query": "not_closed"},
                "comparison": "x",
            },
        ),
        (
            {
                "name": "closed",
                "kind": "closed_form",
                "target": {"kind": "derived", "query": "other"},
            },
        ),
        (
            {
...
def test_query_result_models_reject_invalid_derived_target_nullability():
    ...
    with pytest.raises(ValidationError):
        EquivalenceResult(
            ...
            summary="bad",
            answers=(
                QueryAnswer(conclusion="inapplicable", blockers=("derived target source closed",)),
            ),
        )
...
def test_aggregate_work_comparison_owns_unavailable_cost_and_blocker_policy() -> None:
    ...
    for operand, blocker in (
        ...
        relation = compare_aggregate_work(available, operand, None, semantic_established=True)
    ...
    semantic = compare_aggregate_work(available, unknown, None, semantic_established=False)
