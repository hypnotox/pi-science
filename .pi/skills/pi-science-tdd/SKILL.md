---
name: pi-science-tdd
description: Use to establish the strongest practical durable oracle before a bug fix in pi-science, or to write a failing test before feature implementation when the missing behaviour is testable in isolation.
---
...
2. Exercise the selected oracle before implementation. For an automated bug regression or feature test, run it and confirm it fails for the right reason: `uv run --locked pytest`. For a fix-only alternative, reproduce the unfixed behaviour safely and retain the inputs, expected result, actual evidence, and concrete reason.
...
Route materially larger work through `pi-science-brainstorming` to
