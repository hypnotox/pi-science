...
1. **Orient in the topic.** Invoke `pi-science-orienting` when its repository-truth moments apply. Then run `./awf context <relevant paths>`.
...
4. **Evaluate continuity independently.** Invoke `pi-science-effort-workflow` only when durable continuity materially helps. Brainstorming itself never creates an effort, and design may remain effort-free.
