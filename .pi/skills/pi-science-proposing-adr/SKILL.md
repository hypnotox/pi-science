---
name: pi-science-proposing-adr
description: >
  Use to propose a new ADR under docs/decisions for pi-science.
  Writes the ADR file with required frontmatter and sections, regenerates
  INDEX.md, commits, and hands off to pi-science-reviewing-adr.
---
...
# pi-science-proposing-adr

Writes a new ADR to `docs/decisions/NNNN-kebab-title.md` (status `Proposed`) and commits it. Scope each record to a single load-bearing commitment: one decision per ADR. See `docs/workflow.md` for the full lifecycle rules and `pi-science-adr-lifecycle` for state transitions.
...
When grounding is stale - the ADR will cite repository facts not verified in the current session - invoke `pi-science-orienting` before writing.
...
8. **Terminal step:** invoke `pi-science-reviewing-adr` via the project's skill-invocation mechanism, passing the ADR path. The reviewer applies its lenses and reports findings; route them per the reviewing skill's procedure.
