---
name: pi-science-executing-plans
description: Use to execute a written pi-science plan phase by phase while the parent owns inline transactions.
---
...
1. Resolve the mutable plan and read its complete phase structure. This skill may run without an effort. If continuity independently fired, validate its immutable slug and exact `.awf/efforts/<slug>/memory.md`, confirm canonical YAML `effort: <slug>` identity, preserve one user-managed writer, and use the managed worktree when present; otherwise omit effort and memory fields. If changed facts newly make continuity useful, invoke `pi-science-effort-workflow` before further mutation. A fresh owner assigned one phase or task may run `./awf read plan <plan> <P[.T]>` for its executable closure, including the generated task scope notice, phase-owned Advances and Completes outcomes, Notes, and Phase close; all are phase-owner context only; projection changes neither phase ownership nor checkpoint boundaries and never gives a task helper commit, review, checkpoint, handoff, or outcome authority. Repository sources and current-state documentation remain authoritative, and standalone memory is forbidden.
...
**Required phase evidence.** For generated-prose changes, perform the focused meaning review at the produced-output boundaries and retain the inspected boundaries and result as completion evidence. The parent stages the complete transaction and creates the phase-closing commit only after `./awf check staged` and `./scripts/check` pass; a wired pre-commit hook enforces both at commit time, and the parent runs them manually first only in a clone without wired hooks (checkable with `git config core.hooksPath`; when in doubt, run both manually). The parent performs terminal exhaustive verification after implementation completes. A receipt never substitutes for either parent-owned gate.
