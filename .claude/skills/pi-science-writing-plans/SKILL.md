---
name: pi-science-writing-plans
description: Use to write a change-specific implementation plan for a complex pi-science task under docs/plans. Enforces observable outcomes, authority, boundaries, ordering, focused evidence, and needed confinement. Terminal step invokes pi-science-reviewing-plan.
---
...
- **Gate cost:** `./scripts/check` runs before every commit. Batch closely-related same-shape changes that share one rationale into a single commit; keep genuinely independent concerns separate. A docs-only commit still runs the gate; it just passes quickly with no code to test. See `docs/workflow.md`.
...
- Full plan review reads every ADR from parsed plan-level `adrs:`. If planning surfaces a new load-bearing decision or a correction would contradict linked authority, pause: invoke `pi-science-proposing-adr` (or amend a still-amendable ADR via `pi-science-adr-lifecycle`), complete ordinary ADR review first, then invoke ordinary `pi-science-reviewing-plan` for every affected linked Proposed plan. If implementation already started, reassess completed affected phases and renew their assurance before progression.
