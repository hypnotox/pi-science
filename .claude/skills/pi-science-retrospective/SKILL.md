...
Invoked by `pi-science-effort-workflow` after implementation assurance is settled or explicitly skipped, deferred artifact closure is complete, and managed topology is removed. It runs in the main thread and sees the whole effort.
...
5. **Return the retrospective result.** Report recorded lessons, promotions, and verified topology to `pi-science-effort-workflow`. Do not finish or delete memory; the lifecycle owner finishes last.
