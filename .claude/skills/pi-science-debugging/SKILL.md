---
name: pi-science-debugging
description: >
  Use when investigating a bug or unexpected behaviour in pi-science,
  ...
  pi-science-specific debugging surfaces.
---
...
Any time behaviour is wrong and the cause is not yet known. Skip only for a one-line fix whose cause is obvious and whose strongest practical durable oracle is already established (invoke `pi-science-bugfix` directly in that case). If the bug reveals a load-bearing design gap rather than a straightforward defect, escalate to `pi-science-brainstorming` after confirming the root cause.
...
7. **Hand off.** When the root cause is confirmed and the durable oracle is established, invoke `pi-science-bugfix` for the fix + commit + review discipline. If investigation reveals a design gap rather than a defect, invoke `pi-science-brainstorming` instead.
