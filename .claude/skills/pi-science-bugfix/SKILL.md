---
name: pi-science-bugfix
description: Use when applying a bug fix in pi-science with a known root cause. Writes the regression test first, fixes the root cause, verifies via the gate, commits, and evaluates review need. Self-contained.
---
<!-- GENERATED by awf: do not edit; change .awf/ and run `awf render` -->

# pi-science-bugfix

The fix-and-verify task skill. Companion to `pi-science-debugging`: where debugging owns systematic investigation (finding the root cause), this skill owns the implementation + review pair that ships the fix.
...
If the root cause is not yet known, invoke `pi-science-debugging` first.
...
For materially larger work, route the disposition through `pi-science-brainstorming`:
...
3. **Verify via the gates.** `./scripts/check` is the default. Run `./scripts/check` when regression-test placement warrants the full tier.
