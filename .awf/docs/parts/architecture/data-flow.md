...
Ordinary analysis defaults to three bounded optimization suggestions and accepts `0..16`; zero disables advice. After retained analysis, family-specific local generators feed one Python verifier that expands intermediates by checked substitution, proves retained outputs, and compares whole-computation aggregate work including binder, output, and intermediate multiplicity. Unknown costs, unresolved proof/cardinality, capture, incompatible scopes, and nonpositive or incomparable savings are omitted. Independent inspection, generation, transformation, proof, work, and 65,536-byte advice bounds preserve the prior 262,144-byte base-result allowance; combined Python output is 327,680 bytes and the framed Pi limit is 327,936 bytes. Pi protocol v11 validates strict shape and request/result correlation without recomputing mathematics. Repeated structure/calls/reciprocals, checked factoring, redundant-operation removal, and invariant hoisting ship locally; cross-equation sharing and Horner remain pending.
