...
Python runs one fixed `bounded_goal_v1` fair breadth-first monotonic depth-two search over every shipped exact lane: repeated-structure, call and reciprocal reuse, factoring, redundant-operation removal, iterator-invariant hoisting, compatible cross-equation sharing, Horner reformulation, and `finite_polynomial_sum_v1`. The exact-algorithmic lane shares ADR-0012's checked derivation boundary but replaces only its unique maximal bounded nested finite-polynomial `Sum` tree inside the preserved target shell. Python independently rederives and correlates each algorithmic identity against its replayed parent and rederives every retained identity for direct original-to-final proof; backend candidates and query envelopes are not proof. It projects retained tally dimensions only for the goal-selected objective and publishes only verified positive whole-computation reductions under the submitted domain.
