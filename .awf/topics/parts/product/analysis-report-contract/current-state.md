...
Ordinary successful analysis reports bounded optimization advice separately from submitted interpretation, work, scenarios, and queries. Every published suggestion carries a nonempty tuple of unique target-local transformations, each with its target, normalized original and proposed forms, and structural occurrences. Plans carry canonical objective provenance separate from objective-independent candidate identity. Suggestions carry selected-objective before, after, savings, and one-based qualified adjacent ordering; deterministic non-superiority never claims proved superiority. Ordinary advice and direct optimization expose the same ordered replayable plans; passive optimizer faults preserve base analysis with a bounded failed diagnostic, while direct optimization returns a typed failure without plans. Each plan has one or two complete parent-relative trace states; plan evidence remains direct original-to-final. Search incompleteness names a bounded resource and does not imply no improvement exists. Post-search serialized-output truncation is a separate projection status and qualification that does not erase search qualifications.
Origin: ADR-0017
