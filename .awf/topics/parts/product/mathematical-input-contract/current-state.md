...
Ordinary requests carry `optimization.max_suggestions`, an optional exact objective selector, and an optional `enabled_algorithmic_families` list; direct `optimize` carries `max_plans`, the same selector, and the same family list. Omitted or empty algorithmic selection enables none; the strictly unique canonical list currently accepts only `finite_polynomial_sum_v1`. `weighted_operations_v1` requires all five strictly positive bounded exact-rational operation weights; opaque work remains coefficient one. Returned candidates include syntax, transformed expression or equations, required variables/functions/costs/assumptions/definitions, and output identities, but exclude scenarios, queries, and optimization controls. `max_suggestions` and `max_plans` select only the final ranked output prefix after fixed search; requests expose no depth or budget controls. Candidate comparison and dominance requests do not carry optimization configuration.
Origin: ADR-0017
