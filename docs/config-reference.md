...
| `domains` | string list | none | 0 configured | Freeform domain keys. Each renders a generated `docs/domains/<name>.md` doc (a compact topic list plus your `current-state` convention part) and can declare a file territory via the domain sidecar's `paths:`. | Always. |
...
- `gateCmdFull`: Command for the full/extended gate tier, if the project has one. Consumed while a rendered artifact's template references it.
  State: set (`./awf check`). Consumed by: doc workflow, hooks pre-push, skill bugfix, skill debugging.
...
- `testCmd`: Command that runs the test suite. Consumed while a rendered artifact's template references it.
  State: absent, declined; the generic prose renders. Consumed by: agents-doc, doc workflow, skill tdd.
