...
Kind: batch
Applying: ["separate-formula-responsibilities-behind-compatibility-facades:assign-single-responsibility-owners", "separate-formula-responsibilities-behind-compatibility-facades:direct-python-dependencies", "separate-formula-responsibilities-behind-compatibility-facades:preserve-foundation-compatibility"]
Paths: ["packages/py-science-formula/src/py_science/formula/_analysis/computation.py", "packages/py-science-formula/src/py_science/formula/_analysis/retained.py", "packages/py-science-formula/src/py_science/formula/_service/__init__.py", "packages/py-science-formula/src/py_science/formula/_service/orchestration.py", "packages/py-science-formula/src/py_science/formula/_service/optimization.py", "packages/py-science-formula/src/py_science/formula/_service/query_execution.py", "packages/py-science-formula/src/py_science/formula/_service/scenario_execution.py", "packages/py-science-formula/src/py_science/formula/_service/result_bounds.py", "packages/py-science-formula/src/py_science/formula/service.py", "tests/e2e/test_formula_analysis.py", "tests/e2e/test_formula_system_analysis.py", "tests/e2e/test_formula_candidate_comparison.py", "tests/e2e/test_formula_dominance.py", "tests/unit/test_formula_queries.py", "tests/unit/test_formula_scenarios.py", "tests/unit/test_error_translation.py", "tests/unit/test_public_exports.py", "tests/unit/optimization/test_replay.py", "tests/unit/optimization/test_budgets.py", "tests/distribution/test_python_package.py"]
Representative: Route one ordinary request through retained analysis, service-owned result enrichment for queries and scenarios, passive optimization, and result bounding while preserving its exact report.
...
Post-check: State check: require the unchanged package root and `service.py` to expose the pinned public callables, require `_service` modules to import neutral analysis and `_optimization` owners rather than compatibility facades, require no reverse optimizer edge, and run ordinary, query, scenario, dominance, comparison, replay, and installed-package evidence. Compare serialized scenario-bearing reports and scenario-free replay results before and after the extraction.

The delegated owner must not edit package-root `__init__.py`; the orchestrator verifies its export identity and resolves any semantic conflict outside this phase rather than broadening the phase. Move entry-point orchestration, ordinary and direct optimization dispatch, query attachment and derived-target correlation, scenario specialization, dominance dispatch, and result byte bounding into their named owners. Keep parsing, validation, and retained-computation construction neutral, but introduce an explicit result-enrichment seam in `_analysis/computation.py` so service owns scenario result specialization without duplicating or reparsing neutral analyzer state. Replay uses that seam without acquiring service dependencies and retains scenario-free complete requests. Preserve a top-to-bottom orchestration path and make `service.py` a thin explicit facade for `analyze`, `optimize`, `analyze_dominance`, and characterized compatibility aliases. Do not change request loading, serialized reports, passive versus direct failures, result null policy, report attachment order, optimization disablement during replay, or output allowances.
...
Phase 2 close: commits `a12ccd2` and `30ce4a6` define all former contract objects once behind identity-preserving facades, import every contract module from the installed wheel, and retain schema SHA-256 `58f6648b27c9c1da5ec2670663ff6338a3eb9bbc65f52a433c758bf20e147091`. Phase review found private wildcard exports and incomplete absolute, barrel, and relative import detection. Settlements `20be1ad` and `1947d4a` restore the nonprivate wildcard surface and enforce the reviewed DAG with falsified forbidden forms and positive direct-edge controls. Focused evidence passes 12 cases, the full gate passes 520 Python and 126 Pi tests, and renewed review finds no remaining issue or deviation.

Phase 3 close: commit `692d8ae` splits the strict bridge into six responsibility owners behind an explicit 79-name compatibility barrel, keeps `process.ts` as the lifecycle seam, ships and externally imports every child and the barrel, retains protocol v16 and schema SHA-256 `58f6648b27c9c1da5ec2670663ff6338a3eb9bbc65f52a433c758bf20e147091`, and renders the truthful intermediate architecture. Phase review found one unused duplicate diagnostic bound and an evidence probe that froze optional positive imports. Settlement `5bfcc48` removes the duplicate and checks only authority-backed allowed directions plus the required client-to-process and client-to-correlation seams. Focused evidence passes 88 cases, the full gate passes 520 Python and 129 Pi tests, renewed review finds no residual issue, and the phase audit has no error; its plan-discovery warning is advisory because the reviewed plan predates the phase range.

Phase 4 close: commit `1aad1da` extracts neutral retained facts and occurrence ownership. Review found that comparison still depended on the service analyzer and optimizer still used process-global analyzer fallback. Reasoned settlement `94ea76a` moves the sole parsing, validation, retained construction, and resource-bound entry point to `_analysis/computation.py`; keeps only the characterized service alias; requires a keyword-only per-call optimizer analyzer while preserving the fourth positional budget configuration; and redirects direct tests and moved-owner monkeypatches. Necessary omitted paths were the neutral computation owner, direct optimizer search, budget, and algorithmic-sum tests, and the error-translation and query parsing probes. Fresh verification found only a substring-based dependency probe; mechanical settlement `8f47fd6` replaces it with AST import and signature checks and falsifies absolute, relative, barrel, registry, and optional-analyzer regressions. The full gate passes 526 Python and 129 Pi tests, schema SHA-256 remains `58f6648b27c9c1da5ec2670663ff6338a3eb9bbc65f52a433c758bf20e147091`, and the phase audit has no error; its plan-discovery warning is advisory because this plan predates the phase range. No Phase 4 deviation remains unresolved.

Phase 5 close: commit `18775b0` extracts optimizer generation, budgets, replay, verification, objectives, canonical keys, search, and plan projection behind direct compatibility aliases, with every owner and family imported from the installed wheel. Phase review found duplicate repeated-call admission, four family policies retained by the shared candidate owner, and an import probe that froze optional positive edges. This settlement restores the single specialized call lane, moves cross-equation sharing, Horner materialization, neutral-operation recognition, and factoring policy to their named families, preserves facade alias identity, and narrows DAG evidence to required seams, layer direction, acyclicity, and falsified forbidden forms. These are authority-guided corrections under the frozen population and single-owner decisions; no user decision or plan deviation is required.

Phase 6 approved boundary expansion: Phase 4 made neutral analysis the sole analyzer and left scenario-bearing base results there. To preserve the ADR's service ownership of scenario execution, Phase 6 now includes `_analysis/computation.py` and introduces an explicit result-enrichment seam. Service owns scenario specialization through that seam, while neutral parsing and retained construction, scenario-free optimizer replay, exact serialized reports, and replay behavior remain unchanged. Grounding confirms a keyword-only per-call enricher can receive immutable prepared scenario state and return only scenario results; ordinary service supplies it, while scenario-free replay uses the neutral default. Move scenario result construction and effective-domain specialization to service, keep request validation and shared numeric limits neutral, preserve the consumed work-render budget, and eliminate current scenario-definition reparsing without changing validation order. Added paths cover immutable retained-state reconstruction, moved bound and monkeypatch owners, error translation, and public callable identity.

Phase 6 close: commit `9e75952` extracted the five service owners and kept the facade and root callables identical, with 367 focused cases and the full 532-Python/129-Pi gate passing. Review found reparsed scenario definitions, shallow prepared-state mutability, an unused retained field, stale result-bound monkeypatch assertions, missing service DAG and installed-wheel probes, missing exact scenario serialization and callable-identity evidence, and formatting-only query-test churn. The settlement prepares each scenario once into deep immutable mappings, reuses parsed assumptions and definitions for domain validation and service specialization, removes the unused field, repairs the tests and probes, pins representative serialized output, and restores the query corpus's prior formatting. The suggested scenario-bearing behavior for a bare private neutral analyzer is not adopted: only alias identity is characterized, while a service-enriched neutral default would contradict the approved service ownership and optimizer dependency boundary. The single verify pass found only formatting-only dominance-test churn; the residual settlement restores its prior formatting while retaining the active owner monkeypatch.

Phase 7 close: commit `e7562ae` moved compact result projection to its bridge owner, redirected index and provisioning imports to owning modules, retained the exact compatibility barrel and schema hash `58f6648b27c9c1da5ec2670663ff6338a3eb9bbc65f52a433c758bf20e147091`, shipped every bridge child, and rendered current architecture. The full gate passed 537 Python and 130 Pi tests. Review found that the direct-import probe covered the current integration modules but not every production TypeScript file. Mechanical settlement scans the complete production tree except the barrel, recognizes relative, absolute, and package-qualified direct-barrel specifiers without rejecting bridge children, and failed as expected when temporarily falsified through `process.ts`. No Phase 7 deviation remains unresolved.

Phase 8 close: commit `84b3e72` entered the ADR into Implementing, applied its single responsibility-directed-components claim, and rendered the actual Python and Pi owners, dependency direction, test ownership, and optimizer extension seams. Generated prose meaning review found authored and rendered sources semantically identical, the active-document census found no stale central-file ownership claim, full and release gates passed 537 Python and 131 Pi tests plus installed Python and Pi startup, and schema SHA-256 remained `58f6648b27c9c1da5ec2670663ff6338a3eb9bbc65f52a433c758bf20e147091`. Fresh phase review returned no findings or deviations.
