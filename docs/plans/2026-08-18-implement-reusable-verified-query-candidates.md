...
- Phase-review settlement: corrected qualification composition to use the explicitly named source rather than the preceding result, including non-adjacent provenance and all dependent terminal conclusions; conditions, used assumptions, and relevant unsupported assumptions deduplicate under their public bounds, with overflow unresolved.
- Phase-review settlement: narrowed Python, generated-schema, and TypeScript system target variants so only equivalence and limit consume derived operands; added result-model and strict-bridge conditional-nullability enforcement and negatives.
- Phase-review settlement: completed expression/system, eligibility/structural, unavailable-source, direct-work, schema, and bridge regression coverage. The broad planned pre-implementation red phase was not observed and is not claimed.
- Falsification evidence: before regenerating the artifact, `uv run --locked pytest tests/unit/test_formula_queries.py tests/test_pi_schema_generation.py` collected 40 tests and failed `test_generated_pi_schema_is_deterministic_and_current` because generated bytes differed at index 23537 after system-target narrowing. Regeneration followed; this was an expected schema-artifact failure, not a malformed fixture. Reversible old-composition falsification invoked `_compose_derived_qualification(dependent, unrelated)` for a dependent explicitly naming `closed`; its exact output was `AssertionError: ('unrelated',)`, proving why `results[-1]` was unsound without changing repository state.
- Parent settlement added expression-level and real registered-tool equivalence/limit reuse coverage and tightened the bridge so every non-null normalized target must still be a complete valid interpretation. Focused settlement verification reported 126 Python/schema tests and 56 bridge/registered-tool tests passing before the full gate.
- Verify-pass mechanical settlement requires the unavailable blocker to name the exact derived source and a valid source conclusion; strict bridge correlation additionally matches that conclusion to the referenced earlier closed-form result. Wrong-source, missing-conclusion, and mismatched-conclusion blockers are rejected.
