...
- Phase 2 final freshness settlement: composes resolved global definitions into scenario definition validation and specialization, while rejecting scenario treatments that overlap globally defined variables as conflicting mathematical knowledge. Scenario substitution output remains scenario-local rather than falsely reporting global replacements. Regressions cover global-to-scenario dependency and the previously inconsistent `p = 2` plus scenario `p = 3` case; no approved boundary changed.
- Phase 2 final mechanical residual: recursively closes symbolic global definitions over scenario fixed and finite-choice values before validating or using downstream scenario definitions, and revalidates affected global targets against intrinsic domains. Regressions cover `p = q + 1` with fixed `q`, downstream `r = p + 1`, and a scenario value that would make globally positive `p` negative. No approved boundary changed.
- Phase 3 implementation deviation: added `packages/pi-science/src/provision.ts` and `packages/pi-science/tests/provision.test.ts` to advance and regress the health-check protocol version with the adapter. Task 3.1 requires the protocol advance while preserving health checks and fail-closed mismatch behavior, and ADR-0002 `eager-fail-closed-provisioning` requires readiness validation before surface registration; focused provisioning tests and the complete project gate verify the aligned version and old-version rejection.
- Phase 3 review settlement: made both TypeScript response boundaries reject duplicate JSON object keys and invalid UTF-8, then replaced the generic two-equation acceptance with a shared compact AFMM fixture containing component-indexed displacement, `M[neighbor[b, c], k]` translation, defined `K`, declared `basis` work, and unresolved `translate` cost. The added `packages/pi-science/tests/afmm-fixture.ts` keeps the real bridge and registered-tool assertions identical; adapter, health, total-work, dependency/reuse, provenance, and scenario regressions are green. No approved boundary changed.
- Phase 3 verify-pass residuals: corrected the health-output boundary to reject the first byte beyond the 4,096-byte diagnostic limit and asserted exact normalized renderings for all three AFMM equations across both the real adapter and registered tool. Focused regressions and the complete project gate are green; the fixes are mechanical and do not change the approved boundary.
- Terminal review settlement: made exact assumption replacement and directed-definition dependency discovery respect bounded-sum lexical scope. A bound iterator now shadows same-named free assumption or definition symbols in its body while whole-sum matches and bound substitutions remain available. Regressions cover index-dependent primitive work plus global and scenario definitions whose bound iterator shares a definition name; the complete project gate is green. The correction is mechanical and preserves the approved local-scope boundary.
