...
- Phase 2 test-first evidence deviation: the original phase owner did not preserve or record the required absent-model and post-model absent-service failure sets before committing. That timing evidence cannot be reconstructed. Review instead reproduced false scalar-producer disproof, ignored unequal domains, reversed-target miscorrelation, capture corruption, invalid result-model acceptance, and missing symbolic sign/crossover behavior against the committed implementation.
- Phase 2 review settlement: added complete result truth-table validators, candidate-correlated mappings and localized failures, aggregate request/expansion/result bounds, scalar and indexed capture-avoiding producer expansion, effective-domain proof, submitted-graph work deltas, unknown/non-finite abstention, and existing exact sign-chart winner/crossover evidence. Focused regressions cover reversed mappings, scalar producers, binder capture, unequal domains, recursive expansion overflow, mixed-output precedence, invalid public variants, unknown work, non-finite work, and crossover intervals.
- Phase 2 sign evidence: the named retained-work fixture yields `delta = 1 - N` and exact intervals `(-oo, 1): positive`, `(1, oo): negative`, and `1: zero`, so the result is `crossover`. This exposed and fixed an existing sign-chart witness bug that skipped an inclusive integer-domain endpoint. The scalar-producer fixture yields `delta = -1` and `second_lower` only after semantic equality is proved under `d != 0`.
- Phase 2 documentation evidence: rendered claim review confirms direct Python support, semantic-before-work qualification, ADR-0003 work, explicit abstention, excluded scenarios and queries, and Pi transport still deferred. Both public README comparison examples executed; the named-reuse example proved equality under `d != 0` and retained `delta = -1`.
...
- Phase 3 evidence: protocol census retains only the intentional stale-v8 adapter fixture; generated prose and compact comparison output were inspected after render.
- Phase 3 reasoned deviations: reapply `product/product-boundary:symbolic-analysis-only` alongside the expected input-contract claim because its direct-Python-only wording materially widened to Python and Pi transport under the Phase 3 lifecycle instruction; update these Notes because the Phase close requires the evidence record. `./awf check staged` verifies both authority-guided additions.
- Phase 3 review settlement test-first evidence: the focused schema test failed because comparison candidates lacked an explicit expression/system union; focused bridge tests rejected a real comparison `AnalysisFailure` and accepted invalid interface/work variants; the registered-tool metadata assertion failed because it advertised only ordinary analysis. Each failed for the reviewed omission before its production fix.
- Phase 3 review settlement: accept comparison `AnalysisFailure` envelopes, enforce strict candidate schema unions, mapped-output and work truth tables, request/result correlation, source bounds, and real adapter/bridge/registered-tool coverage. Synchronize the package guide, analysis model, glossary, testing guide, tool metadata, and skill routing description with protocol v9 Pi transport. The full gate passed with 312 Python and 98 Pi tests, plus schema generation, pyright, ruff, TypeScript, formatting, and awf checks.
- Phase 3 renewed-review settlement: model canonical expression-candidate `analysis.system: null` explicitly in the TypeScript transport type and cover the real adapter shape. The regression first failed type checking because the earlier type allowed only an absent or populated system. The renewed full gate passed with 312 Python and 99 Pi tests.
- Terminal assurance settlement: synchronize the primary README's Pi workflow with the shipped exactly-two-candidate request, semantic-before-work rule, second-minus-first delta, abstention, and exclusions. Fresh phase reviews had covered package and generated guidance but not this cross-phase entry point.
