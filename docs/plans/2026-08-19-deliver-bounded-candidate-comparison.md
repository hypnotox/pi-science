...
- Phase 1 review settlement: made retained parsed-equation and work-analysis state deeply read-only, added parser-call characterization proving expression and system queries reuse validated operands, restored the `_attach_queries` docstring, and reformatted the introduced retained-state paths. The late baseline record is a reasoned process deviation; no approved design boundary changed.
