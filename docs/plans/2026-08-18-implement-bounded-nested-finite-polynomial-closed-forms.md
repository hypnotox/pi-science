...
- Phase-owner test-first evidence: the focused baseline passed with 126 Python and 72 Pi tests; after the nested regression was added it failed because the evaluator still rejected nested sums, and the final phase-owner run passed with 127 Python and 72 Pi tests.
- Phase-owner semantic-review evidence: inspected the rendered architecture, testing, roadmap, and both current-state topics; the readings labeled the family partial, kept candidates informational rather than submitted work, retained protocol-v7/Python policy ownership, and inventoried excluded extensions.
- Renewed-review settlement: preflight every descendant before an empty-ancestor short circuit, keep bound binders lexical under directed definitions, use the nested intermediate ceiling rather than total degree eight in backend generation, and add collision, absence, zero/leading-negative, overflow, per-intermediate, empty-ancestor, and multivariate per-binder regressions.
- Final-freshness settlement: fail closed when a directed replacement would capture a lexically bound sum name, and enforce the depth-four/eight-node topology against the capture-safe reasoning-expanded tree; definition-introduced depth and node overflow now have focused regressions.
- Targeted-freshness settlement: make definition expansion path-lexical so sibling binders do not cause false capture, cover direct and two-hop capture plus a safe sibling case, and enforce one shared intermediate construction budget with a reversible budget-bypass falsification.
- Final mechanical settlement: apply a Sum binder only to its body, not its lower or upper expressions, matching the repository's lexical substitution semantics; a same-named free limit resolved by a directed definition now has a focused regression.
- Terminal-assurance settlement: derive qualification provenance from replacements actually applied under lexical Sum scope, retain bound-definition provenance for empty ranges, and add the planned query-bearing harmonic-system comparison, including unchanged work, scenarios, reuse, output multiplicity, and quartic primitive invocations.
