...
- Terminal review settlement: both Python adopter paths now inspect PEP 610 `direct_url.json` and require their resolved commit IDs to equal the selected snapshot; bridge response retention uses a byte-bounded buffer and discards further output during cleanup, with a resistant high-volume regression. Reversible mutations independently broke both commit-ID assertions and ready/disabled skill discovery; all four failed at their owning assertions and were exactly restored.
