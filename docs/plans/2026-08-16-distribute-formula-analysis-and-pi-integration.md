...
- Phase 3: `npm run test:pi -- --run packages/pi-science/tests/start.test.ts` first failed because ready resource discovery returned `undefined`; after registering the readiness-gated product skill path it passed. `./scripts/check-release` snapshots tracked edits and intended untracked files into a temporary commit, proves the separate Python and Pi pins resolve to it, and exercises clean Python installation plus the ready/disabled Pi resource contract. Its source-pin assertion was reversibly changed from `== "$sha"` to `== "not-the-snapshot"`; it failed at `==> source pins resolve ...` with exit status 1, then was restored exactly to `== "$sha"` before final gates.
- Phase 3 review settlement: corrected the documented Pi source to Pi's supported URL-at-ref syntax; changed the bridge to derive both immutable commit and repository origin from Pi's managed checkout; and made `scripts/check-release` drive the real project Git-package manager and noninteractive RPC host, assert healthy/disabled tool and skill discovery, and execute persistent and PEP 723 Python pins. Public verification is executable as `--public-ref <tag> --expected-sha <sha>`. The resistant-descendant oracle timeout was raised from 20 ms to 250 ms so the wrapper records its PID before the tested timeout boundary.
- Phase 3 residual settlement: the missing-`uv` RPC probe now uses only explicitly linked executables, while dedicated readers continuously drain RPC stdout and bounded stderr and process-group cleanup bounds failures. Reversible negative probes separately broke the public-ref SHA expectation, persistent Python result, PEP 723 result, managed-checkout SHA, ready tool discovery, and disabled tool absence; each failed at its owning assertion and was exactly restored before final verification.
- Terminal review settlement: both Python adopter paths now inspect PEP 610 `direct_url.json` and require their resolved commit IDs to equal the selected snapshot; bridge response retention uses a byte-bounded buffer and discards further output during cleanup, with a resistant high-volume regression. Reversible mutations independently broke both commit-ID assertions, ready/disabled skill discovery, and the retained response prefix; all five failed at their owning assertions and were exactly restored.
