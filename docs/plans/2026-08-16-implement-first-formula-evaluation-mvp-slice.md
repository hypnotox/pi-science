...
Paths: ["src/pi_science/", "tests/e2e/test_formula_evaluation.py", "tests/unit/test_error_translation.py"]
...
Paths: ["scripts/check", ".gitignore", ".awf/config.yaml", ".awf/parts/agents-doc/identity.md", "glob:.awf/docs/parts/architecture/*.md", "glob:.awf/docs/parts/development/*.md", "glob:.awf/docs/parts/testing/*.md", ".awf/docs/parts/debugging/surfaces.md", ".awf/docs/parts/releasing/content.md", "docs/analysis-model.md", "glob:docs/architecture.md", "glob:docs/development.md", "glob:docs/testing.md", "docs/debugging.md", "docs/releasing.md", "docs/config-reference.md", "docs/workflow.md", ".awf/hooks/pre-commit.sh", ".awf/hooks/pre-push.sh", ".awf/awf.lock", "AGENTS.md", "CLAUDE.md", "glob:.claude/**", "glob:.pi/**"]
...
- Implementation deviation: the deny-by-default parser introduced for the first supported expression already rejected later unsupported categories, so their end-to-end cases confirmed green behavior rather than each requiring another production change; every newly implemented arithmetic operator was still witnessed red before its parser support landed.
- Implementation deviation: added `.gitignore`, debugging, and releasing paths after the stale-claim sweep found environment artifacts depended on a user-global ignore and two generated documents still called the repository documentation-only. These paths are required by the approved reproducible toolchain and current-documentation outcomes.
- Phase review settlement: normalized empty and non-UTF-8 input into typed malformed-syntax failures; added the user-approved 64 KiB UTF-8 input, 4,096-node internal, depth-128, and approximately 1,024-decimal-digit integer budgets with consumer-facing messages; and disabled eager SymPy power evaluation. Internal node-budget exhaustion uses only `expression is too complex`.
- Phase review settlement: moved broad third-party exception wrapping into the SymPy adapter with exception chaining and made the service catch only the named `NormalizationError`; focused unit tests prove cause preservation and that unrelated programming errors propagate.
- Phase review settlement: reversible probes produced nonzero verdicts independently for pytest, Pyright, Ruff, and awf, after which each temporary mutation was removed and the complete gate returned green.
- Renewed phase review settlement: protected unevaluated powers from eager evaluation by every parent operator, rejected oversized decimal integers before Python's interpreter parser ceiling, and counted signed integer literals as one prospective internal expression node. Focused regressions cover every parent operator, a 5,000-digit literal, and both sides of the signed-literal node boundary.
- Terminal review settlement: extended decimal-token limits to valid oversized zero-only and underscore-separated zero literals. Review also found the initial feature commit omitted its dependency-justification body; the reviewed history remains append-only rather than amended, while the approved Architecture summary and Task 1.1 retain the toolchain rationale and this note records the convention exception.
