...
- Phase 3 review settlement keeps caller-output identities distinct from generated producer equations, clears both passive projections on advice overflow, and bounds duplicated direct-plan output by retaining only complete plans that fit and returning an incomplete qualification. It removes syntax at the Pi candidate projection, requires the v13 plan population, and strictly correlates candidate context, caller outputs, stable identity, suggestion, requested limit, and complete/incomplete qualifications. The registered tool presents direct plans and typed failures before ordinary-analysis rendering. Active architecture and agent guidance describe v13 replay and current bounds. The direct-output trimming is an authority-determined reasoned correction inside the approved bounded-result contract; all other corrections are mechanical. Added dominance and provisioning fixtures remain required atomic protocol-migration coverage. No material-scope deviation was required.
- The Phase 3 verify pass found that output identities made the returned candidate fail strict unchanged replay and that result trimming skipped the full and empty retained populations. The settlement accepts validated output identities as ordinary-analysis replay metadata, omits only the inactive computation branch in the Pi projection, and tests expression and generated-producer system replay through the real adapter. It also selects the largest fitting plan prefix, including all or zero plans. These corrections follow the approved complete-candidate and bounded-result contracts; no new decision or material-scope deviation was required.
