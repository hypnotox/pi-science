...
- Phase 1 renewed review: removed generator-invented expression length constraints because `AnalysisRequest.model_json_schema()` does not publish them; the existing bridge byte bound and Python diagnostics remain authoritative.
- Terminal review: changed adapter diagnostic truncation to discard only an incomplete trailing UTF-8 code point and added a real-adapter boundary regression, preserving the 4,096-byte bridge contract for multibyte Python messages.
