...
- Phase 4 settlement: added `tests/unit/test_formula_properties.py` because the named query test file is already a broad Phase 2/3 contract suite; an isolated focused matrix gives each new valid-domain, singularity, sign-chart, directional-limit, monotonicity, and terminal distinction a direct regression boundary. Review corrections replace the AFMM shortcut with exact approved-tail matching, retain submitted exclusions separately from canceled poles, orient poles through the local denominator cofactor, preserve participating provenance, support signed parameter factors and factorized symbolic exclusions conservatively, return complete active-domain chart partitions, and confine every transformation to bounded backend seams. These changes enforce the accepted exact/qualified boundary without adding a mathematical family.
