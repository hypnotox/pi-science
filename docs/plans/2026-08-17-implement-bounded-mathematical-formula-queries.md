...
- Phase 1 settlement: scenarios over non-finite direct work fail with source identity and the supported alternative to remove scenarios, because the existing scenario result contract represents only work specializations and cannot report a null work value. Regression fixtures in `tests/unit/test_formula_scenarios.py` are a necessary added path. Review fixes also preserve decimal source lexemes, enforce exact IR and direct-work variant invariants, localize parser diagnostics, and correlate the protocol-v3 validators.
