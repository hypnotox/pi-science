...
- Phase 3 review settlement: preserve negative-power denominator obligations, prove symbolic ordering and ratio exclusions from bounded affine facts with participating provenance, move guarded construction and independent finite/infinite identity checks behind the bounded backend seam, and exercise verification failure and resource boundaries. Family-specific bounded collection now recognizes algebraically presented sums such as `k*q**k + q**k` as the already-approved normalized `(a*k+b)*r**k` family; this reasoned correction follows the plan's normalized-summand requirement without widening the mathematical family.
