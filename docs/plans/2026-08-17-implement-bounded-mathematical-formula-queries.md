...
- Phase 1 settlement: scenarios over non-finite direct work fail with source identity and the supported alternative to remove scenarios, because the existing scenario result contract represents only work specializations and cannot report a null work value. Regression fixtures in `tests/unit/test_formula_scenarios.py` are a necessary added path. Review fixes also preserve Unicode-safe decimal source lexemes and maximum-size scalar fractions, enforce pre- and post-reduction exact-value bounds and nested direct-work invariants, give canonical integral rationals the same equality, cardinality, and exponent semantics as integers, reserve `oo` across mathematical identifiers, classify direct and every definition-substituted infinity as non-finite work, reject infinity recursively in finite output domains, primitive work, and scenario work definitions, preserve required protocol nulls, retain actual end-exclusive diagnostic spans in UTF-8 byte columns while leaving unavailable precision null, and correlate the protocol-v3 validators through the report hierarchy.
- Phase 2 review settlement: replace the permissive result and transport envelopes with exact discriminated variants, preserve required nullable keys, and validate every nested protocol-v4 query shape. Move definition, equality, domain, affine-inequality, provenance, and assignment checks into the bounded reasoning owner; retain original denominator obligations before guarded rational normalization; enforce pre-call, intermediate, coefficient, rendering, request, and result bounds; and add unit, end-to-end, adapter, and negative transport evidence. These corrections implement the approved Phase 2 boundary without changing its supported family.
- Phase 3 review settlement: preserve negative-power and enclosing submitted-denominator obligations; prove symbolic ordering and ratio exclusions from bounded affine facts with participating replacement provenance; and move guarded construction, finite step/boundary verification, and infinite exact-tail verification behind the bounded backend seam. Family-specific bounded normalization recognizes additive, subtractive, and index-independent factored presentations of the already-approved `(a*k+b)*r**k` family without widening it. Focused falsification covers incorrect candidates, submitted singularities, and pre-call resource refusal; generated candidates retain the separate 4096-node intermediate budget after the 512-node target preflight.
- Phase 4 settlement: added `tests/unit/test_formula_properties.py` because the named query test file is already a broad Phase 2/3 contract suite; an isolated focused matrix gives each new valid-domain, singularity, sign-chart, directional-limit, monotonicity, and terminal distinction a direct regression boundary. Review corrections replace the AFMM shortcut with exact approved-tail matching, retain submitted and parameter-only denominator obligations separately from canceled poles, orient poles through the local denominator cofactor, and confine every transformation to bounded backend seams. Exact root ordering, operation-local provenance, proved-real applicability, active-domain chart cells, and factor-wise affine sign composition preserve conservative results for symbolic factors while rejecting coefficients whose sign varies over their assumptions. These changes enforce the accepted exact/qualified boundary without adding a mathematical family.
- Phase 5 settlement: added `tests/unit/test_asymptotics.py` to isolate rational-recurrence and exponential-decomposition falsification from the shared query-contract suite. Review corrections handle exact zero, variable-specific local parameters, declared-real rational coefficients, multi-base linear-exponential terms, per-source-term order, original conditions and provenance, and real target/coefficient bounds. All polynomial translation, truncation, reconstruction, cancellation, and rendering now pass through bounded backend seams and must independently verify before proof; this implements the approved matrix without generic series operations or work-growth coupling.
- Phase 6 settlement: review fixes add `nonnegative_real` to both Pi schemas, apply declared domains to intervals by nonempty intersection rather than endpoint membership, and reuse bounded query reasoning for affine and equality assumption intersections. Equality replacements also intersect declared-domain facts. Constant scenario definitions reject negative `nonnegative_real` values. Added scenario and registered-tool regressions cover these boundaries without enabling scenario queries.
- Phase 7 settlement: TypeBox now distinguishes expression queries (no target) from system queries (required equation target), and reserves `oo` across every query identifier position. The bridge correlates successful result count, order, name, kind, resolved target, and property checks with the submitted request while retaining mathematical conclusions as opaque backend output. The final mechanical settlement makes those comparisons structural rather than JSON-order-sensitive, distinguishes defined expression context, carries the distinction into TypeScript request types, and adds system/reordered-key regression plus compile-time contract evidence. Focused temporary falsification disabled each new schema discriminator and response-name comparison and caused its focused regression to fail before restoration. Terminal assurance also requires every system request to return a system report; a single expression may legitimately retain its existing nested-system report for iterator analysis. No mathematical semantics changed.
