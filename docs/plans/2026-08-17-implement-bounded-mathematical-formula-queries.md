...
- Phase 6 settlement: review fixes add `nonnegative_real` to both Pi schemas, apply declared domains to intervals by nonempty intersection rather than endpoint membership, and reuse bounded query reasoning for affine and equality assumption intersections. Constant scenario definitions reject negative `nonnegative_real` values. Added scenario and registered-tool regressions cover these boundaries without enabling scenario queries.
