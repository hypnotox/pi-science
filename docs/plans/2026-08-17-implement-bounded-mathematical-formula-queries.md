...
- Phase 4 deviation: added `tests/unit/test_formula_properties.py` because the named query test file is already a broad Phase 2/3 contract suite; an isolated focused matrix gives each new valid-domain, singularity, sign-chart, directional-limit, monotonicity, and terminal distinction a direct regression boundary. This remains inside Task 4.1 verification scope and adds no mathematical family; its focused pytest run passes.
