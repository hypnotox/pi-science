...
Phase 3 expands its paths to `packages/pi-science/tests/package.test.ts` because the shipped-skill package characterization asserted removed passive-advice wording and must track the explicit goal guidance in the same documentation transaction. The four declared claim updates are Applied with no remaining operation, every generated surface is current, active guidance removes the legacy optimization controls, and capability-evidence links resolve. The focused package population passes 7 tests, the full gate passes 546 Python and 122 Pi tests, and the release gate passes source-pin installation, packaged startup, readiness, and diagnostics. Phase review found two stale analysis/comparison-only architecture summaries and over-specific shipped-skill prose assertions, and requested falsification evidence. Settlement names the complete operation union, checks minimal durable literals plus removed legacy controls, and proves the package oracle fails when `bounded_goal_v1` is temporarily corrupted before returning to 7 passes.

Terminal cross-phase review found that Pi request-aware correlation treated an explicitly `undefined` required goal objective as unit work. Although reported as a user decision, the approved all-literals-required contract and active input claim determine the correction without changing scope: `undefined` is invalid, while explicit `{kind: "unit_work_v1"}` remains valid. A test failed first because the malformed in-memory request was accepted, then passed after the one-line canonicalizer fix; the full gate remains green.
