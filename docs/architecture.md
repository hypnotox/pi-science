...
Ordinary analysis defaults to three bounded optimization suggestions, accepts `0..16`, and uses zero to disable advice. Python alone generates repeated-structure/call/reciprocal reuse, checked factoring, redundant-operation removal, iterator-invariant-hoisting, compatible cross-equation-sharing, and bounded Horner candidates. Each generated proposal now becomes one internal complete computation: lexical sharing uses `Let`, while output or system-wide sharing uses a complete named system. Python reparses and validates that candidate through the ordinary retained-computation seams before it proves retained outputs and compares binding-aware whole aggregate work; proof-only expansion happens after validation and never supplies candidate work. Candidates are independent, so advice does not compose them. The public surface remains ordinary advice only: protocol v12 continues to project its existing target-local suggestion without exposing an optimize operation or v13 shape. Pi transports and presents that v12 projection without mathematical policy; Python owns candidate construction, proof, work, ranking, and base-result independence. Exact-symbolic reassociation makes no finite-precision claim.
