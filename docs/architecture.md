...
Ordinary analysis defaults to three bounded optimization suggestions, accepts `0..16`, and uses zero to disable advice. Python alone generates local repeated-structure/call/reciprocal reuse, checked factoring, redundant-operation removal, and iterator-invariant-hoisting candidates, expands intermediates for exact-symbolic proof, and requires a positive whole-computation aggregate-work reduction. Independent search and advice bounds preserve the ordinary report; Pi protocol v11 validates and presents the qualification without recomputing policy. Cross-equation sharing and Horner remain pending, and exact-symbolic reassociation makes no finite-precision claim.
