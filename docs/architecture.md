...
Ordinary analysis defaults to three bounded optimization suggestions, accepts `0..16`, and uses zero to disable advice. Python alone generates repeated-structure/call/reciprocal reuse, checked factoring, redundant-operation removal, iterator-invariant-hoisting, compatible cross-equation-sharing, and bounded Horner candidates, expands intermediates for exact-symbolic proof, and requires a positive whole-computation aggregate-work reduction. Independent search and advice bounds preserve the ordinary report; Pi protocol v12 validates without recomputing policy and compactly presents the first-ranked proved complete target-local transformation set, intermediate when present, savings, conditions, and finite-precision qualification, with additional suggestions in canonical details; deterministic presentation is not a superiority claim. Exact-symbolic reassociation makes no finite-precision claim.
