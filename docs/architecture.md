...
- `packages/pi-science/src/bridge/`: protocol primitives, request and result shapes, bounded diagnostics, request-aware correlation, and the per-call adapter client state machine.
- `packages/pi-science/src/bridge.ts`: compatibility barrel still consumed by retained presentation and provisioning integration; bridge children never import it.
- `packages/pi-science/src/process.ts`: adapter process spawning and process-tree termination mechanisms.
- `packages/pi-science/src/index.ts`: generated-schema tool registration, result presentation, routing metadata, and always-available doctor.
