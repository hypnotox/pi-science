...
A free equation-output index needs a local domain. A `Sum` iterator is local to its body. Every external symbol needs an intrinsic variable domain. A named equation is charged once per local output-domain point; downstream references are accesses to that result, while unnamed repetition remains submitted work. Nested finite direct-work fields preserve each iterator under its lexical symbolic `Sum`, including operation counts, opaque work, and primitive invocations. Inclusive sums use nonnegative cardinality `max(upper - lower + 1, 0)` when declared integer and sign facts make that reasoning valid; an unproved cardinality remains an explicit unresolved qualification rather than a free iterator.
