...
Finite points require `left`, `right`, or `both`; `oo` and `-oo` forbid direction. An expression query omits `target`. A system query selects one named equation RHS with `{ "kind": "equation", "name": "..." }`. An `equivalence` or `limit` may instead spell `{ "kind": "derived", "query": "earlier_closed_form" }`, naming only an earlier `closed_form` query; closed-form, properties, and asymptotic consumers never accept that target. The named source is eligible only when its sole answer is proved (possibly under assumptions), carries checked closed-form evidence, and has exactly one candidate. A dependent inherits and deduplicates the source conditions, used assumptions, and relevant unsupported assumptions within public bounds; overflow is unresolved. If the source is unavailable, the dependent remains correlated, is `inapplicable`, has a null normalized target, and names the source conclusion in its blocker. There is no fallback to the submitted expression or equation RHS. Derived operands preserve their source provenance but never change submitted work, direct-work aggregation, or scenarios. Queries use global assumptions only and do not execute in scenario contexts.
