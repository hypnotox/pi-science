...
7. an explicit preserve-all optimization goal with fixed bounded depth-two search over every shipped exact-algebraic family and the bounded nested finite-polynomial `Sum` family; every published plan has independent proof, a `strict_improvement` claim, and positive whole-computation selected-objective reduction.
