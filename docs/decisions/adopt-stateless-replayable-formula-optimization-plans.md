...
1. `decision: explicit-stateless-optimization-operation` Python and Pi will accept an explicit bounded `operation: optimize` alongside ADR-0017 ordinary default-on advice. Its sole initial operation control is `max_plans`, which defaults to 3 and accepts strict integers from 1 through 16; configurable families and search depth remain outside this decision.
2. `decision: shared-optimization-policy` The explicit operation and ordinary advice will use the same Python-owned retained computation, candidate generation, verification, cost, deduplication, and ranking policy. Ordinary advice projects the same verified plans rather than maintaining an independent optimizer or result policy.
3. `decision: complete-replayable-plans` Every public optimization plan will contain one authoritative complete candidate: the transformed expression or equation system, the declared formula context required to interpret it, and its output identities. The candidate mirrors the caller-facing ordinary-analysis input at each boundary: direct Python includes its required syntax discriminator, while Pi uses the established syntax-injected projection. Analysis-only scenarios, queries, and optimization controls are not candidate semantics and do not travel inside the candidate. An agent can resubmit the candidate without reconstructing hidden placement, consulting server state, or modifying its mathematical content. Target-local descriptions may remain only as bounded diagnostics.
4. `decision: minimal-lexical-binding` The restricted project-owned expression model will add one bounded `Let(name, value, body)` form whose name is a bare capture-free symbol. The binding is nonrecursive: the value is interpreted in the enclosing lexical and equation-output environment, and the fresh name is visible only in the body; generated names and substitutions are capture-avoiding. The value is evaluated once for each evaluation of the construct in that enclosing environment, while references in the body are reuse accesses. Nesting expresses additional bindings, and moving a binding across an aggregate or output scope changes its evaluation multiplicity. The construct is valid both as analyzer input and optimizer output and supports expression-local, aggregate-local, iterator-dependent, output-dependent, and hoisted intermediates without adding a general scheduling IR.
...
7. `decision: backend-independent-transport` Optimization semantics and typed results remain transport-free and Python-owned. Pi protocol v13 will strictly validate, transport, and present the operation, replayable plans, and failure variants without recomputing binding scope, equivalence, work, applicability, ranking, or failure policy.
