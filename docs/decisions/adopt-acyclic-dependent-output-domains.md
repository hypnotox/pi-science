...
3. `decision: bounded-relational-domain-reasoning` Dependent-domain acceptance, cardinality, aggregate direct work, and scenario specialization use an explicit bounded family of integer-domain and relational deductions. The initial family admits affine integer bounds and only supported deterministic inference; every used relationship retains provenance, while unsupported, cyclic, unbounded, or unproved domains are rejected or reported as qualified and unresolved rather than delegated to unrestricted symbolic reasoning.
