...
One fixed profile cannot express an agent's exact symbolic preference between the five already-retained operation dimensions. Existing candidates can differ across those dimensions: under unit work, factoring `(x + 1)*(x + 1)` saves more than extracting the repeated `x + 1` from a separate subexpression, while a sufficiently larger exact power weight reverses that order and leaves both improvements positive. The retained dimensional tally can support that choice without adding runtime, hardware, storage, scheduling, or numerical policy.
