...
3. `decision: mathematical-aggregate-work-semantics` A bounded sum has inclusive bounds, multiplies its body work by its provable nonnegative integral cardinality, and adds the mathematical reduction cost of one fewer addition than terms. Nested sums apply the same rule recursively. Aggregate work excludes loop control, indexing, bound evaluation, storage, and hardware effects, and remains qualified or unresolved when the required cardinality facts cannot be established. Reports distinguish submitted structural operations from aggregate executed mathematical work.
...
5. `decision: provenance-preserving-system-analysis` Assumptions and definitions are safely parsed mathematical relationships applied only through supported deterministic transformations, with cycles and directly detectable contradictions rejected. Every simplification identifies the relationships it used; unsupported inference and untreated scaling variables remain unresolved. Results expose normalized SymPy and LaTeX interpretations, per-equation and system work, dependencies and reuse, primitive invocation counts, assumptions used, unknown costs, and qualified conclusions without claiming physical correctness, implementation timing, or global algorithmic optimality.
...
The public request, result, and subprocess protocol must evolve together and enforce whole-request complexity and bounded-output policies. LaTeX parsing, cyclic recurrences, physical validation, source analysis, empirical performance, hardware cost models, and code generation remain outside this decision.
