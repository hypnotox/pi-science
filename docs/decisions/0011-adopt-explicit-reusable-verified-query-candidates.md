...
3. `decision: verified-candidate-applicability` A derived target is usable only when a `proved` or `proved_under_assumptions` source answer carries checked closed-form verification evidence and exactly one candidate. A successful dependent result reports that candidate as its normalized target. When a structurally valid source runs but does not provide that operand, the dependent query remains correlated to the request, returns `inapplicable` with a null normalized target and a blocker naming the source and its conclusion, and never silently analyzes the submitted target instead.
...
The request and result target contracts must distinguish submitted and derived operands. Consumers must handle a null normalized target only for an unavailable explicit derived operand rather than assuming every valid query resolved an expression. The exact private protocol must advance with the changed wire contract.
...
- 2026-08-18: Implemented; content-sha256: 34eb5968159301c622645c186afcbb2fa36fe9fab46c6f74897d56a94fce367b
