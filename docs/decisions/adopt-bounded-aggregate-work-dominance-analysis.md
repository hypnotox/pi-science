...
1. `decision: single-axis-original-work-dominance` Add a domain-neutral operation that analyzes terms within one computation's original reuse-aware aggregate abstract-work expression. The request selects exactly one declared scaling variable, may fix other scalar variables exactly, and may restrict the axis to an exact open or closed interval; omission uses the declared domain. It does not compare whole candidates or alter ordinary analysis results.
2. `decision: canonical-rational-power-terms` For the bounded supported family, reduce work to one checked univariate rational form and define stable signed terms as the collected numerator power terms over its shared reduced denominator. Require exact reconstruction before publishing the decomposition, compare term relevance by absolute magnitude while retaining signed renderings, and abstain for unsupported coefficients, opaque aggregates, exponentials, unknown costs, or unresolved work.
3. `decision: exact-domain-aware-dominance-regions` Derive piecewise dominance only through bounded exact sign reasoning with an explicit selected axis. Partition at proved magnitude crossovers, term zeros, denominator poles, and active-domain boundaries; report complete tied maximum sets, excluded points, never-dominant terms, and localized unresolved regions. Real axes use exact intervals and admissible points, integer axes use coalesced admissible integer ranges and points, and a proved-empty active domain returns a successful empty result.
4. `decision: qualified-bounded-dominance-transport` Bound term population, pairwise comparisons, structural reasoning, partition growth, rendering, and the supplemental plus combined report before quadratic or serialized growth escapes existing ceilings. Python owns decomposition and mathematical conclusions; Python and Pi expose matching strict requests and reports through the existing analysis product surface. No result claims runtime, storage, scheduling, hardware behavior, rewrite safety, optimization, or multivariate dominance.
