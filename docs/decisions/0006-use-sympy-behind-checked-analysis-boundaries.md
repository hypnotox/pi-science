...
1. `decision: preserve-project-owned-analysis-policy` Keep the typed mathematical model, resource gates, assumptions and provenance, work and reuse semantics, supported-query policy, qualification, and fail-closed outcomes authoritative in `py-science-formula`; no unrestricted or unverified CAS result defines public support or constitutes proof.
2. `decision: prefer-checked-sympy-mechanics` Use SymPy as the preferred algebra, rendering, and verification engine behind explicit family checks and resource preflight and postflight rather than recreating backend mathematics.
...
The project retains adapter and verification code around SymPy and may expose less functionality than SymPy can compute generally. This cost buys predictable resource use, inspectable evidence, and freedom to change backend mechanisms without changing the analysis contract.
